...
Полный журнал хранится в `logs/nfqws-debug.log`, а однозначно помеченные события профилей автоматически раскладываются по `logs/profiles/profile-N-name.log`. Если общий размер всех журналов достигает 200 МБ, они автоматически очищаются. Завершив проверку, вернитесь в обычный режим командой `sudo ./start.sh`.

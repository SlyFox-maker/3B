...
После открытия NFQUEUE процесс сбрасывает права до числовых `NFQWS2_UID`/`NFQWS2_GID` (по умолчанию официальные значения tester-а `1:3003`). Явное указание обязательно для сборок nfqws2, которые иначе выбирают некорректный UID.
